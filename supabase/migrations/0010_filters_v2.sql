-- ============================================================================
-- 0010_filters_v2.sql
-- The sidebar filter set grew well past the original eight scalar params:
-- the UI now multi-selects provinces and price buckets, supports excluded
-- categories, lets the user pick how null ratings are treated, gates by
-- review-count range, and toggles has-phone/website/address/coordinates.
--
-- Rather than overload the RPCs (PostgREST can't disambiguate by name in
-- our setup), drop and recreate every filter-aware function with the new
-- signature. Each one shares the same composable WHERE pattern below so
-- behavior stays consistent across overview / map / categories / column /
-- export paths.
--
-- The unfiltered short-circuit (overview_cache, map_cache) is preserved by
-- comparing every new param against its "no-op" default. The export RPC is
-- new — keyset-paginated so the /api/export-csv route can stream the full
-- result of any filter combination without materialising it in memory.
-- ============================================================================

-- Drop old signatures first (parameter types changed, so plain CREATE OR
-- REPLACE would be ambiguous). The map cache wrapper depends on the
-- compute fn, so cascade those.
drop function if exists public.restaurants_overview(
    text, text, text[], text, boolean, integer, numeric, numeric, text
) cascade;
drop function if exists public.restaurants_overview_compute(
    text, text, text[], text, boolean, integer, numeric, numeric, text
) cascade;
drop function if exists public.restaurants_map_points(
    text, text, text[], text, boolean, integer, numeric, numeric, text, integer
) cascade;
...
drop function if exists public.restaurants_column_stats(
    text, text, text, text[], text, boolean, integer, numeric, numeric, text, integer
) cascade;

-- ----------------------------------------------------------------------------
-- Composable WHERE: every filter-aware RPC inlines the same predicate set.
-- Postgres folds the constant-null checks at plan time, so unused filters
-- have zero runtime cost.
--
-- p_rating_null_policy: 'include' | 'exclude' | 'only'
--   include: a row with rating IS NULL passes the score range
--   exclude: only rows with non-null rating in [min,max] pass
--   only:    only rows with rating IS NULL pass (range ignored)
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- restaurants_map_points (slim columnar payload)
-- ============================================================================
...
-- ============================================================================
-- restaurants_top_categories (filter-aware)
-- ============================================================================
...
-- ============================================================================
-- restaurants_export_page (NEW)
-- Keyset-paginated export. The /api/export-csv route fetches successive
-- batches of (default) 5000 rows and streams them to the client as CSV.
-- Keyset on id keeps each call O(p_limit) regardless of how deep we are.
-- ============================================================================
...
-- ============================================================================
-- Refresh caches so the unfiltered short-circuit returns a value computed
-- under the new signatures. Without this the existing cache rows would be
-- inconsistent with the new compute fns (same JSON shape, but it's still
-- worth recomputing once on deploy so refresh_at is current).
-- ============================================================================
