-- ============================================================================
-- 0026_approval_status_exclude.sql
-- Add a separate `p_excluded_statuses` array to every approval-aware RPC so
-- the exclude side of the approval-status filter can be applied independently
-- of the include side.
--
-- Before this migration the client merged include + exclude into a single
-- "effective include" list and sent it as p_approval_statuses. That broke
-- whenever a row carried more than one status (e.g. approved + downloaded):
-- excluding "approved" would still leave the row matching the include list
-- via "downloaded", so it leaked through into the map and the counts.
--
-- After this migration a row passes when
--     (p_approval_statuses is null OR matches include)
--   and (p_excluded_statuses is null OR not matches exclude)
-- and stacked-status leaks are gone.
-- ============================================================================

-- Drop the previous signatures (they don't have p_excluded_statuses).
...
as $$
    with filtered as MATERIALIZED (
        ...
        where (p_provinces          is null or province     = any(p_provinces))
          ...
          and (
              p_excluded_statuses is null
              or not public.approval_status_matches(
                  id, p_excluded_statuses,
                  p_approved_ids, p_rejected_ids, p_skipped_ids,
                  p_downloaded_ids, p_sent_to_sheets_ids
              )
          )
...
as $$
    select case
        when p_provinces          is null
         ...
         and p_excluded_statuses  is null
        then coalesce(
...
-- Refresh caches under the new signatures (data shape unchanged).
