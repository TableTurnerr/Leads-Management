-- ============================================================================
-- 0023_lead_status_filter.sql
-- Push the lead-status filter into the SQL so map / overview / categories /
-- column-explorer / export RPCs all narrow results consistently. The state
-- (which IDs are approved / rejected / etc.) lives client-side in
-- localStorage, so the client ships the relevant ID arrays as RPC params.
--
-- p_lead_statuses encodes which statuses pass the filter. Status semantics
-- mirror getLeadStatuses() in web/src/lib/lead-status.ts:
--   - Every row has exactly one of {approved, rejected, skipped, pending}
--     (first non-empty wins; pending = none of the others).
--   - Rows may additionally be downloaded and/or sentToSheets.
-- A row passes when any of its statuses is in p_lead_statuses (OR-inclusion).
-- Passing null for p_lead_statuses disables the filter and re-enables the
-- cache fast-path on the no-op call.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- Helper: lead_status_matches(id, statuses, approved, rejected, skipped,
--                             downloaded, sent_to_sheets)
-- Marked immutable + parallel safe so the planner can inline this into the
-- enclosing WHERE clause and fold the null/empty cases at plan time.
-- ----------------------------------------------------------------------------
create or replace function public.lead_status_matches(
    p_id                 bigint,
...
as $$
    with filtered as MATERIALIZED (
        ...
        where (p_provinces          is null or province     = any(p_provinces))
          ...
          and public.lead_status_matches(
              id, p_lead_statuses,
...
as $$
    select case
        ...
        then coalesce(
            ...
            p_lead_statuses, p_approved_ids, p_rejected_ids,
